...
def load_results_to_dataframe():
    """Load all result files and convert to simple pandas DataFrame"""
    results_dir = 'results'
    data_list = []
                    ...
                    if isinstance(results, list) and len(results) > 0:
                        ...
                        if isinstance(result_tuple, tuple) and len(result_tuple) == 4:
                            ...
                            if isinstance(student_results, dict) and 'acc_1hot' in student_results:
                                ...
                                # Create a simple row with only the core columns
                                row = {
                                    'width': width,
                                    'depth': depth,
                                    'auxiliary': auxiliary,
                                ...
                                print(f"No acc_1hot found in {filename}")
                        else:
...
def save_dataframe(df, filename='results_simple.csv'):
